% 类似接口函数，提供VRP接口
function [path, time, allMile, num] = Export(numNode, x, y, demand, numCar, capacity, timeLimit, k1, k2, k3, Callback)
    ...
    % 添加车辆信息
    for i = 1:numCar
        vrp.addCar(capacity(i), timeLimit(i));
    end
    ...
    % 调用回调函数
    [time, allMile] = Callback(numCar, path, pathLen, load, mileage, time);
end
